...
// Registration describes a reusable blueprint example that can be advertised or instantiated.
...
// NewBlueprint returns a model for this registration.
func (r Registration) NewBlueprint() *Blueprint {
	if r.Build != nil {
		return r.Build()
	}

	return &Blueprint{
		Name:        normaliseBlueprintName(r.Name),
		Kind:        normaliseBlueprintKind(r.Kind),
		Description: strings.TrimSpace(r.Description),
		Status:      BlueprintStatusDraft,
	}
}

// Registry stores blueprint registrations by key.
...
func (r *Registry) Register(entry Registration) error {
	if r == nil {
		return ErrBlueprintRegistrationNotFound
	}

	entry.Key = normaliseRegistrationKey(entry.Key)
	if entry.Key == "" {
		return ErrBlueprintRegistrationKeyMissing
	}
	entry.Name = normaliseBlueprintName(entry.Name)
	if entry.Name == "" {
		return ErrBlueprintNameIsRequired
	}
	entry.Kind = normaliseBlueprintKind(entry.Kind)
	if entry.Kind == "" {
		return ErrBlueprintKindIsRequired
	}
	...
	if _, exists := r.entries[entry.Key]; exists {
		return ErrBlueprintRegistrationConflict
	}
...
func (r *Registry) MustGet(key string) (Registration, error) {
	...
	if !exists {
		return Registration{}, ErrBlueprintRegistrationNotFound
	}
