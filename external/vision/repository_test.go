...
func (m *mockMongoStore) ExecuteFindOneCommandDecodeResult(ctx context.Context, collection *mongo.Collection, filter interface{}, result interface{}, resultObjectName string, logError bool, onFailureErr error) error {
	...
	if vision, ok := result.(*Vision); ok {
		*vision = Vision{ID: "bp-1", Name: "Example", Kind: "demo"}
	}
	return nil
}
...
func (m *mockMongoStore) MapAllInCursorToResult(ctx context.Context, cursor *mongo.Cursor, result interface{}, resultObjectName string) error {
	if visions, ok := result.(*[]Vision); ok {
		*visions = []Vision{{ID: "bp-1", Name: "Example", Kind: "demo"}}
	}
	return nil
}

func TestRepositoryGetVisionCollection(t *testing.T) {
	tests := []struct {
		name                string
		initErrs            []error
		dbErrs              []error
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantErr {
				if err == nil {
					t.Fatal("GetVisionCollection() expected error, got nil")
				}
				if !errors.Is(err, ErrVisionDatabaseError) {
					t.Fatalf("GetVisionCollection() error = %v, want ErrVisionDatabaseError", err)
				}
				return
			}
			if err != nil {
			...
			if collection.Name() != VisionCollection {
				t.Fatalf("collection name = %s, want %s", collection.Name(), VisionCollection)
			}
			...
			if tt.secondCallUsesCache {
				_, err = repo.GetVisionCollection(context.Background())
				if err != nil {
					t.Fatalf("second GetVisionCollection() error = %v", err)
				}
				if store.initialiseClientCalls != tt.wantInitCallCount {
...
func TestRepositoryCreateVisionUsesVisionCollection(t *testing.T) {
	store := &mockMongoStore{}
	repo := NewRepository(store)

	created, err := repo.CreateVision(context.Background(), &Vision{ID: "bp-1", Name: "Example", Kind: "demo"})
	if err != nil {
	...
	if store.lastCollectionName != VisionCollection {
		t.Fatalf("collection = %s, want %s", store.lastCollectionName, VisionCollection)
	}
}
...
func TestBuildVisionListFilter(t *testing.T) {
	tests := []struct {
		name string
		req  *GetVisionsRequest
		want map[string]interface{}
	}{
		{name: "SUCCESS - nil request", req: nil, want: map[string]interface{}{}},
		{name: "SUCCESS - kind filter", req: &GetVisionsRequest{Kind: " Demo "}, want: map[string]interface{}{"kind": "demo"}},
		{name: "SUCCESS - status filter", req: &GetVisionsRequest{Status: " Active "}, want: map[string]interface{}{"status": "active"}},
		{name: "SUCCESS - query filter", req: &GetVisionsRequest{Query: " Starter API "}, want: map[string]interface{}{"$or": []bson.M{
		...
		{name: "SUCCESS - combined filters", req: &GetVisionsRequest{Kind: " Demo ", Status: " Active "}, want: map[string]interface{}{"kind": "demo", "status": "active"}},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := buildVisionListFilter(tt.req)
