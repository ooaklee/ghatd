package billingmanager

import "errors"

var (
	ErrBillingManagerFailedToProcessEvent                  = errors.New(ErrKeyBillingManagerFailedToProcessEvent)
	ErrBillingManagerFailedToRetrieveBillingEvents         = errors.New(ErrKeyBillingManagerFailedToRetrieveBillingEvents)
	ErrBillingManagerFailedToRetrieveSubscriptionStatus    = errors.New(ErrKeyBillingManagerFailedToRetrieveSubscriptionStatus)
	ErrBillingManagerFailedWebhookVerification             = errors.New(ErrKeyBillingManagerFailedWebhookVerification)
	ErrBillingManagerNoUserIdentifyingInformationInPayload = errors.New(ErrKeyBillingManagerNoUserIdentifyingInformationInPayload)
	ErrBillingManagerPricerServiceNotSet                   = errors.New(ErrKeyBillingManagerPricerServiceNotSet)
	ErrBillingManagerRequiresUserIdIsMissing               = errors.New(ErrKeyBillingManagerRequiresUserIdIsMissing)
	ErrBillingManagerUnableToGetProviderNameFromURI        = errors.New(ErrKeyBillingManagerUnableToGetProviderNameFromURI)
	ErrBillingManagerUnableToGetUserIdFromURI              = errors.New(ErrKeyBillingManagerUnableToGetUserIdFromURI)
	ErrBillingManagerUnableToIdentifyUser                  = errors.New(ErrKeyBillingManagerUnableToIdentifyUser)
	ErrBillingManagerUnableToResolveUserId                 = errors.New(ErrKeyBillingManagerUnableToResolveUserId)
	ErrBillingManagerUserUnauthorisedToCarryOutOperation   = errors.New(ErrKeyBillingManagerUserUnauthorisedToCarryOutOperation)
	ErrBillingManagerCheckoutProviderUnavailable           = errors.New(ErrKeyBillingManagerCheckoutProviderUnavailable)
	ErrBillingManagerCheckoutConfigurationInvalid          = errors.New(ErrKeyBillingManagerCheckoutConfigurationInvalid)
	ErrBillingManagerCheckoutOriginRejected                = errors.New(ErrKeyBillingManagerCheckoutOriginRejected)
	ErrBillingManagerCheckoutPriceUnavailable              = errors.New(ErrKeyBillingManagerCheckoutPriceUnavailable)
	ErrBillingManagerCheckoutPriceAmbiguous                = errors.New(ErrKeyBillingManagerCheckoutPriceAmbiguous)
	ErrBillingManagerCheckoutTermsUnsupported              = errors.New(ErrKeyBillingManagerCheckoutTermsUnsupported)
	ErrBillingManagerCheckoutUserUnavailable               = errors.New(ErrKeyBillingManagerCheckoutUserUnavailable)
	ErrBillingManagerCheckoutCatalogueUnavailable          = errors.New(ErrKeyBillingManagerCheckoutCatalogueUnavailable)
	ErrBillingManagerCheckoutProviderRequestFailed         = errors.New(ErrKeyBillingManagerCheckoutProviderRequestFailed)
	ErrBillingManagerCheckoutSessionInvalid                = errors.New(ErrKeyBillingManagerCheckoutSessionInvalid)
	ErrBillingManagerCheckoutIdempotencyFailed             = errors.New(ErrKeyBillingManagerCheckoutIdempotencyFailed)
	ErrBillingManagerPortalProviderUnavailable             = errors.New(ErrKeyBillingManagerPortalProviderUnavailable)
	ErrBillingManagerPortalConfigurationInvalid            = errors.New(ErrKeyBillingManagerPortalConfigurationInvalid)
	ErrBillingManagerPortalOriginRejected                  = errors.New(ErrKeyBillingManagerPortalOriginRejected)
	ErrBillingManagerPortalBillingUnavailable              = errors.New(ErrKeyBillingManagerPortalBillingUnavailable)
	ErrBillingManagerPortalSubscriptionUnavailable         = errors.New(ErrKeyBillingManagerPortalSubscriptionUnavailable)
	ErrBillingManagerPortalProviderRequestFailed           = errors.New(ErrKeyBillingManagerPortalProviderRequestFailed)
	ErrBillingManagerPortalSessionInvalid                  = errors.New(ErrKeyBillingManagerPortalSessionInvalid)
	ErrBillingManagerPortalCustomerAmbiguous               = errors.New(ErrKeyBillingManagerPortalCustomerAmbiguous)
	ErrInvalidBillingManagerRequestPayload                 = errors.New(ErrKeyInvalidBillingManagerRequestPayload)
)
