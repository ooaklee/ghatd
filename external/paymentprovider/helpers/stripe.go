...
// configure applies defaults and optionally validates the constructed provider.
func (s *StripeSettings) configure(configuration StripeConfiguration, validateProvider bool) error {
	...
	if apiKeyMode == "" {
		return stripeHelperError(ErrStripeKeyModeInvalid, "stripe_api_key must use a recognised Stripe test or live mode prefix")
	}
	publishableKeyMode := stripePublishableKeyMode(s.StripePublishableKey)
	if publishableKeyMode == "" {
		return stripeHelperError(ErrStripeKeyModeInvalid, "stripe_publishable_key must use a recognised Stripe test or live mode prefix")
	}
...
// trimValues removes incidental whitespace from every configurable value.
...
// resolveFrontendPath joins a trusted frontend base URL to an application path.
...
// validateReturnURL enforces the configured frontend origin and HTTPS policy.
...
// validateStripeAPIBaseURL restricts non-local traffic to Stripe's public API origin.
...
// stripeAPIKeyMode identifies the test or live mode of a Stripe secret key.
...
// stripePublishableKeyMode identifies the test or live mode of a publishable key.
...
// parseAbsoluteHTTPURL parses a safe absolute HTTP or HTTPS URL.
...
// canonicalHTTPOrigin returns a lower-case origin with default ports removed.
...
// isLocalEnvironment reports whether relaxed local URL policy is permitted.
...
// stripeHelperError wraps a stable error kind with safe configuration detail.
